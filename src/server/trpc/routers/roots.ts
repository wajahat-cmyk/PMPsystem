...
import { eq, and, gte, lte, sql, desc, countDistinct } from "drizzle-orm";
...
export const rootsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const datePrev = dateDaysAgo(input.days * 2);
      ...
      const currentRows = await db
        .select({
          rootId: keywordDailyMetrics.rootId,
          rootTerm: sql<string>`COALESCE(${roots.rootTerm}, 'No Root')`.as("root_term"),
          syntaxGroupCount: countDistinct(keywordDailyMetrics.syntaxGroupId).as("syntax_group_count"),
        ...
        .where(whereClause)
        .groupBy(keywordDailyMetrics.rootId, roots.rootTerm)
        .orderBy(desc(sql`SUM(CAST(${keywordDailyMetrics.spend} AS NUMERIC))`));

      // Previous period metrics for WoW deltas
      const prevConditions = [
        gte(keywordDailyMetrics.date, datePrev),
        lte(keywordDailyMetrics.date, dateFrom),
      ];
      if (input.productId) {
        prevConditions.push(eq(keywordDailyMetrics.productId, input.productId));
      }
      const prevWhereClause = and(...prevConditions);
      ...
      const prevRows = await db
        ...
        .groupBy(keywordDailyMetrics.rootId);

      const prevMap = new Map<number | null, { spend: number; sales: number }>();
      for (const row of prevRows) {
        prevMap.set(row.rootId, {
      ...
      const items = currentRows.map((row) => {
        ...
        const wastedSpend = Number(row.wastedSpend) || 0;

        const ctr = impressions > 0 ? (clicks / impressions) * 100 : 0;
        const cvr = clicks > 0 ? (orders / clicks) * 100 : 0;
        const acos = sales > 0 ? (spend / sales) * 100 : 0;
        const wasPct = spend > 0 ? (wastedSpend / spend) * 100 : 0;

        const prev = prevMap.get(row.rootId);
        ...
        const acosDelta = prevAcos > 0 ? acos - prevAcos : 0;

        return {
          rootId: row.rootId,
          rootTerm: row.rootTerm,
          syntaxGroupCount: Number(row.syntaxGroupCount) || 0,
          ...
          acosDelta: Math.round(acosDelta * 100) / 100,
        };
      });
  ...
  getSyntaxGroups: protectedProcedure
    .input(
      z.object({
        rootId: z.number(),
        days: z.number().min(1).max(90).default(7),
    ...
    .query(async ({ input }) => {
      ...
      const conditions = [
        gte(keywordDailyMetrics.date, dateFrom),
        eq(keywordDailyMetrics.rootId, input.rootId),
      ];
      if (input.productId) {
        conditions.push(eq(keywordDailyMetrics.productId, input.productId));
      }
